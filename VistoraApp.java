...
public class VistoraApp {
    ...
    static final class HotelService {
        ...
        int roomCount(){return rooms.size();} int activeReservationCount(){return (int)reservations.stream().filter(r->!r.isCancelled()).count();} List<Reservation> getReservations(){return List.copyOf(reservations);} List<Room> getRooms(){return List.copyOf(rooms);} void save(){database.save(new HotelData(rooms,reservations));}
    }
